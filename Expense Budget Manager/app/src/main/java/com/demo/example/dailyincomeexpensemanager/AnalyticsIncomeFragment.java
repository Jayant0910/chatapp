...
public class AnalyticsIncomeFragment extends BaseFragment {
    ...
    private void PrepareDataForChart() {
        ...
        this.incomeCategoryList = myDatabaseHandler.getCategoryList(2);
        ...
        this.incomeAmount = new Double[this.incomeCategoryList.size()];
        this.incomeName = new String[this.incomeCategoryList.size()];
        this.incomeColor = new String[this.incomeCategoryList.size()];
        for (int i2 = 0; i2 < this.incomeCategoryList.size(); i2++) {
            this.incomeAmount[i2] = Double.valueOf(Double.parseDouble(this.incomeCategoryList.get(i2).getCategoryTotal()));
            this.incomeName[i2] = this.incomeCategoryList.get(i2).getName();
            this.incomeColor[i2] = this.incomeCategoryList.get(i2).getColor();
            this.yvalues.add(new PieEntry(Float.parseFloat(this.incomeCategoryList.get(i2).getCategoryTotal()), this.incomeCategoryList.get(i2).getName()));
            this.xVals.add(this.incomeCategoryList.get(i2).getName());
            this.colors.add(Integer.valueOf(Color.parseColor(this.incomeCategoryList.get(i2).getColor())));
            this.barEntries.add(new BarEntry((float) i2, Float.parseFloat(this.incomeCategoryList.get(i2).getCategoryTotal())));
        }
        try {
    ...
    private void setPieChart() {
        ...
        this.pieChart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
            ...
            public void onValueSelected(Entry entry, Highlight highlight) {
                ...
                if (AnalyticsIncomeFragment.this.getCurrencySymbol().equals("¢") || AnalyticsIncomeFragment.this.getCurrencySymbol().equals("₣") || AnalyticsIncomeFragment.this.getCurrencySymbol().equals("₧") || AnalyticsIncomeFragment.this.getCurrencySymbol().equals("﷼") || AnalyticsIncomeFragment.this.getCurrencySymbol().equals("₨")) {
                    str = String.format("%.2f", Double.valueOf(value)) + AnalyticsIncomeFragment.this.getCurrencySymbol();
                } else {
                    str = AnalyticsIncomeFragment.this.getCurrencySymbol() + String.format("%.2f", Double.valueOf(value));
                }
