package com.anass.ninflix.Activities.Jdownload;
...
public class DownloadManage {
    ...
    public static void updateNotifications(Download download){
        if(download.getStatus() == Status.DOWNLOADING || download.getStatus() == Status.QUEUED){
            String title = download.getHeaders().get("anime") + " - Episode " + download.getHeaders().get("episode");
    ...
    public void addDownloadRequest(String url, File file, String title, String epetitle){
        ...
        fetch.getDownloads(new Func<List<Download>>() {
            ...
            public void call(@NonNull List<Download> result) {
                ...
                if(exist || file.exists()){
                    ...
                    request.addHeader("episode", epetitle.replaceAll("episode", "").trim());
    ...
    private void downloadAlreadyExist(){
        Toast.makeText(context, "This Episode Has Already Downloaded", Toast.LENGTH_SHORT).show();
    }
